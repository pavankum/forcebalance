...
class EnergyLevelsTarget(Target):
    ...
    def get_internal_coords(self, shot, positions):
        ...
        v_ic = {
            ...
            "angles": np.array(
                [ic.value(positions) * RADIAN_2_DEGREE for ic in ic_dict["ic_angles"]]
            ),
            "dihedrals": np.array(
                [
                    ic.value(positions) * RADIAN_2_DEGREE
                    for ic in ic_dict["ic_dihedrals"]
                ]
            ),
            "impropers": np.array(
                [
                    ic.value(positions) * RADIAN_2_DEGREE
                    for ic in ic_dict["ic_impropers"]
                ]
            ),
        }
        return v_ic
    ...
    def _setup_scale_factors(
        self, bond_denom=0.0, angle_denom=0, dihedral_denom=100, improper_denom=0
    ):
    ...
    def indicate(self):
        title_str = (
            "Energy Levels: %s, Objective = % .5e, Units = kcal/mol, Angstrom"
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def custom_loss(
            reference_list, predicted_list, order_weight=10.0, difference_weight=1.0
        ):
            """
            ...
            for a, b in indcs:
                ddE = (predicted_list[a] - predicted_list[b]) - (
                    reference_list[a] - reference_list[b]
                )
                ddE = np.abs(ddE)
        ...
        def compute(mvals_, indicate=False):
            ...
            for i in range(self.ns):
                ...
                if self.calc_ic:
                    ...
                    diff_bond = (
                        (abs(vref_bonds - vtar_bonds) * self.scale_bond).tolist()
                        if self.n_bonds > 0
                        else []
                    )
                    ...
                    diff_angle = (
                        (
                            abs(periodic_diff(vref_angles, vtar_angles, 360))
                            * self.scale_angle
                        ).tolist()
                        if self.n_angles > 0
                        else []
                    ...
                    diff_dihedral = (
                        (
                            abs(periodic_diff(vref_dihedrals, vtar_dihedrals, 360))
                            * self.scale_dihedral
                        ).tolist()
                    ...
                    diff_improper = (
                        (
                            abs(periodic_diff(vref_impropers, vtar_impropers, 360))
                            * self.scale_improper
                        ).tolist()
                    ...
                    sys_obj_list = (
                        diff_bond + diff_angle + diff_dihedral + diff_improper
                    )
                    # store
                    all_diff[i] = dict(
                        bonds=diff_bond,
                        angle=diff_angle,
                        dihedral=diff_dihedral,
                        improper=diff_improper,
                    )
                    ...
                    current_rmsd = dict(
                        bonds=compute_rmsd(vref_bonds, vtar_bonds),
                        angle=compute_rmsd(vref_angles, vtar_angles, v_periodic=360),
                        dihedral=compute_rmsd(
                            vref_dihedrals, vtar_dihedrals, v_periodic=360
                        ),
                        improper=compute_rmsd(
                    ...
                    try:
                        ...
                        if hasattr(self, "emm_orig"):
                            ...
                            for xi, yi, tx in zip(
                                ...
                                if y_seen:
                                    flag = np.any(
                                        np.full(len(y_seen), yi) - y_seen < 0.2
                                    )
                                if flag:
                                    ax.annotate(
                                        tx,
                            ...
                            for xi, yi, tx in zip(
                                ...
                                if y_seen:
                                    flag = np.any(
                                        np.full(len(y_seen), yi) - y_seen < 0.05
                                    )
                                if flag:
                                    ax2.annotate(
                                        tx,
                                ...
                                y_seen.append(yi)
                            ax2.plot(
                                [11] * len(self.rmsd_orig[dsort]),
                                self.rmsd_orig[dsort],
                                "_",
                                label="RMSD Init.",
                                markersize=20,
                                linewidth=2,
                        ...
                        lgnd = ax.legend(
                            handles=handles_, loc="upper right", prop={"size": 8}
                        )
            ...
            return custom_loss(self.eqm, compute.emm)
