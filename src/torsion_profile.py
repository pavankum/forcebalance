...
from forcebalance.nifty import (eqcgmx, printcool, printcool_dictionary,
                                warn_press_key)
...
class TorsionProfileTarget(Target):
    ...
    def indicate(self):
        title_str = (
            "Torsion Profile: %s, Objective = % .5e, Units = kcal/mol, Angstrom"
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def custom_loss(
            ...
            indcs = itertools.combinations(range(len(predicted_list)), 2)
            relative_e_err = []
        ...
        def compute(mvals_, indicate=False):
            ...
            return custom_loss(
                self.eqm, compute.emm
            )  # (np.sqrt(self.wts)/self.energy_denom) * (compute.emm - self.eqm)
